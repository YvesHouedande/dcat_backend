...
const commandesService = {
    ...
    return await db
      .transaction(async (tx) => {
        try {
          ...
          const commandeInserted = await tx
            .insert(commandes)
            .values({
          ...
          if (commandeData.produits && commandeData.produits.length > 0) {
            for (const produitItem of commandeData.produits) {
              ...
              const productDetails = await tx
                .select({ prix_produit: produits.prix_produit })
                .from(produits)
              ...
              if (productDetails.length === 0) {
                throw new Error(
                  `Produit avec ID ${produitItem.id_produit} non trouvé.`
                );
              }
        ...
        try {
          ...
          if (result.client) {
            emailNotificationService
              .sendCommandeConfirmationToClient(result.commande, result.client)
              .catch((err) =>
                console.error("Erreur d'envoi notification email client:", err)
              );
          }
          ...
          if (result.admins && result.admins.length > 0) {
            emailNotificationService
              ...
              .catch((err) =>
                console.error("Erreur d'envoi notification email admin:", err)
              );
          }
          ...
          try {
            ...
            console.log(
              `Panier vidé pour le client ${result.commande.id_client} après la commande ${result.commande.id_commande}`
            );
          } catch (err) {
            console.error(
              `Erreur lors du vidage du panier pour le client ${result.commande.id_client}:`,
              err
            );
  ...
  getClientCommandes: async (clientId) => {
    try {
      ...
      const commandesWithTotal = await Promise.all(
        commandesList.map(async (commande) => {
  ...
  updateCommandeStatus: async (id, newStatus) => {
    ...
    if (!validStatuses.includes(newStatus)) {
      throw new Error(
        `État de commande invalide. Les valeurs autorisées sont: ${validStatuses.join(
          ", "
        )}`
      );
    }
    ...
    if (notificationMessage) {
      ...
      await emailNotificationService
        .notifyClient(existingCommande[0].id_client, {
        ...
        .catch((err) =>
          console.error("Erreur d'envoi notification (status):", err)
        );
      ...
      if (
        ...
        await emailNotificationService
          ...
          .catch((err) =>
            console.error("Erreur d'envoi notification admin (status):", err)
          );
      }
    }
  ...
  updateLivraisonDate: async (id, dateLivraison) => {
    ...
    await emailNotificationService
      .notifyClient(commandeExistante[0].id_client, {
        type: "date_update",
      ...
      .catch((err) =>
        console.error("Erreur d'envoi notification (date):", err)
      );
  ...
  updateCommandeStatusAndDate: async (id, newStatus, dateLivraison = null) => {
    ...
    const dateChanged =
      dateLivraison && (!oldDate || oldDateFormatted !== newDateFormatted);
    ...
    if (dateLivraison) {
      // Mettre à jour la date seulement si fournie
    ...
    if (newStatus === "livree" && statusChanged) {
      ...
      notificationMessage =
        "Votre commande est maintenant en cours de traitement.";
      notificationType = "status_update";
    } else if (newStatus === "en_cours" && dateChanged && !statusChanged) {
      // Cas de validation avec date
    ...
    if (notificationMessage) {
      await emailNotificationService
        ...
        .catch((err) =>
          console.error("Erreur d'envoi notification (combined):", err)
        );
    }
  ...
  getCommandesStatsByStatus: async () => {
    try {
      const stats = await db
        ...
        .from(commandes)
        .groupBy(commandes.etat_commande);
      ...
      stats.forEach((stat) => {
        if (statsObject.hasOwnProperty(stat.etat_commande)) {
          statsObject[stat.etat_commande] = parseInt(stat.count) || 0;
        }
      });

      // Calculer le total
      const total = Object.values(statsObject).reduce(
        (sum, count) => sum + count,
        0
      );
      ...
      throw new Error("Impossible de récupérer les statistiques des commandes");
    }
  },
};
