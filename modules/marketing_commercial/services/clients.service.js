...
const { clients_en_ligne, refresh_tokens } = require("../../../core/database/models");
...
const clientsService = {
  ...
  register: async ({ nom, email, contact, password }) => {
    ...
    if (!contactRegex.test(contact)) {
      throw new Error("Le numéro de contact doit être au format international (+CodePaysNuméro)");
    }
    ...
    const existingUser = await db.select().from(clients_en_ligne)
      .where(
        or(
    ...
    const newUser = await db.insert(clients_en_ligne).values({
      nom: nom.trim(),
  ...
  requestPasswordReset: async (email) => {
    ...
    const resetUrl = `${process.env.FRONTEND_URL || 'https://boutique.dcat.ci'}/reset-password?token=${resetToken}`;
  ...
  resetPassword: async (token, newPassword) => {
    ...
    const tokenRecord = await db.select()
      .from(refresh_tokens)
      .leftJoin(clients_en_ligne, eq(refresh_tokens.user_id, clients_en_ligne.id_client))
