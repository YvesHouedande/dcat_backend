...
async function getCommandeProducts(commandeId) {
  try {
    const result = await db
      ...
      .innerJoin(
        produits,
        eq(commande_produits.id_produit, produits.id_produit)
      )
    ...
    Object.keys(mainImagesByProductId).forEach((productId) => {
      mainImagesByProductId[productId].sort(
        (a, b) => (a.numero_image || 999) - (b.numero_image || 999)
      );
    });
    ...
    const productsWithImages = result.map((product) => {
      ...
      const mainImage =
        productImages.length > 0 ? productImages[0].lien_image : null;
    ...
    console.error(
      "Erreur lors de la récupération des produits de la commande:",
      error
    );
    return [];
  }
...
const emailNotificationService = {
  ...
  sendEmail: async (to, subject, htmlContent, options = {}) => {
    try {
      const mailOptions = {
        ...
        subject:
          options.withBranding !== false ? `${subject} - Boutique` : subject,
  ...
  notifyClient: async (clientId, notification) => {
    ...
    try {
      ...
      switch (notification.type) {
        ...
        case "commande_update":
          if (notification.newStatus === "livree") {
            ...
            htmlContent = notificationTemplates.delivered(
              clientName,
              notification
            );
            ...
            htmlContent = notificationTemplates.confirmed(
              clientName,
              notification
            );
          } else if (notification.newDate) {
            // Commande validée (implicitement car newDate est défini)
            ...
            // Mise à jour de statut générique (sans date)
            subject = `Mise à jour de votre commande`;
            htmlContent = notificationTemplates.generic(
              clientName,
              notification
            );
          }
      ...
      if (shouldShowStepIndicator) {
        const stepIndicator = generateStepIndicator(
          notification.newStatus || "en_attente",
  ...
  sendCommandeConfirmationToClient: async (commande, client) => {
    ...
    try {
      ...
      let produitsHTML =
        "<p>Chargement des détails des produits en cours...</p>";
      ...
      try {
        ...
        console.error(
          "Erreur lors de la récupération des produits pour l'email:",
          error
        );
      ...
      const htmlContent = createOrderConfirmationTemplate(
        commande,
        client,
        produitsHTML
      );
  ...
  sendCommandeNotificationToAdmin: async (commande, client, admins) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Erreur lors de la récupération des produits pour email admin:",
          error
        );
        produitsHTML = `
          <div class="error-products">
      ...
      console.error(
        "Erreur d'envoi d'email de notification aux admins:",
        error
      );
      return false;
    }
  ...
  // Envoyer une notification d'annulation par email au client
  sendCommandeCancellationToClient: async (commande, client) => {
    if (!client || !client.email) {
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (produits && produits.length > 0) {
          ...
          produitsHTML = `
            <div class="no-products">
              <p>⚠️ Aucun produit trouvé dans votre commande annulée.</p>
              <p>Veuillez consulter votre compte pour les détails complets ou contactez notre service client.</p>
            </div>
          `;
        }
        ...
        console.error(
          "Erreur lors de la récupération des produits pour l'email d'annulation:",
          error
        );
        ...
        produitsHTML = `
          <div class="error-products">
            <p>📋 Le récapitulatif détaillé des produits n'est pas disponible dans cet email.</p>
            <p>Veuillez consulter votre espace client pour voir tous les détails de votre commande annulée.</p>
            <p>Si vous avez des questions, n'hésitez pas à nous contacter.</p>
          </div>
        `;
      }

      const htmlContent = createOrderCancellationTemplate(
        commande,
        client,
        produitsHTML
      );
      ...
      const emailSent = await emailNotificationService.sendEmail(
        client.email,
        "Annulation de votre commande chez DCAT",
        htmlContent
      );

      return emailSent;
    } catch (error) {
      console.error("Erreur d'envoi d'email d'annulation au client:", error);
      return false;
    }
  },

  // Envoyer une notification d'annulation par email aux administrateurs
  sendCommandeCancellationToAdmin: async (commande, client, admins) => {
    if (!admins || admins.length === 0) {
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (produits && produits.length > 0) {
          ...
          produitsHTML = `
            <div class="no-products">
              <p>⚠️ Aucun produit trouvé pour cette commande annulée.</p>
              <p>Veuillez vérifier les détails dans l'interface d'administration.</p>
            </div>
          `;
        }
        ...
        console.error(
          "Erreur lors de la récupération des produits pour email admin (annulation):",
          error
        );
        produitsHTML = `
          <div class="error-products">
      ...
      const htmlContent = createAdminCancellationTemplate(
        commande,
        client,
        produitsHTML
      );
      ...
      console.error("Erreur d'envoi d'email d'annulation aux admins:", error);
      return false;
    }
  },

  // Envoyer une notification "en cours" par email au client
  sendCommandeInProgressToClient: async (commande, client) => {
    if (!client || !client.email) {
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (produits && produits.length > 0) {
          ...
          produitsHTML = `
            <div class="no-products">
              <p>⚠️ Aucun produit trouvé dans votre commande en cours.</p>
              <p>Veuillez consulter votre compte pour les détails complets ou contactez notre service client.</p>
            </div>
          `;
        }
        ...
        console.error(
          "Erreur lors de la récupération des produits pour l'email en cours:",
          error
        );
        ...
        produitsHTML = `
          <div class="error-products">
            <p>📋 Le récapitulatif détaillé des produits n'est pas disponible dans cet email.</p>
            <p>Veuillez consulter votre espace client pour voir tous les détails de votre commande en cours.</p>
            <p>Si vous avez des questions, n'hésitez pas à nous contacter.</p>
          </div>
        `;
      }

      const htmlContent = createOrderInProgressTemplate(
        commande,
        client,
        produitsHTML
      );
      ...
      const emailSent = await emailNotificationService.sendEmail(
        client.email,
        "Votre commande est en cours de traitement chez DCAT",
        htmlContent
      );

      return emailSent;
    } catch (error) {
      console.error("Erreur d'envoi d'email en cours au client:", error);
      return false;
    }
  },

  // Envoyer une notification "en cours" par email aux administrateurs
  sendCommandeInProgressToAdmin: async (commande, client, admins) => {
    if (!admins || admins.length === 0) {
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (produits && produits.length > 0) {
          ...
          produitsHTML = `
            <div class="no-products">
              <p>⚠️ Aucun produit trouvé pour cette commande en cours.</p>
              <p>Veuillez vérifier les détails dans l'interface d'administration.</p>
            </div>
          `;
        }
        ...
        console.error(
          "Erreur lors de la récupération des produits pour email admin (en cours):",
          error
        );
        produitsHTML = `
          <div class="error-products">
      ...
      const htmlContent = createAdminInProgressTemplate(
        commande,
        client,
        produitsHTML
      );
      ...
      console.error("Erreur d'envoi d'email en cours aux admins:", error);
      return false;
    }
  },

  // Envoyer une notification d'annulation/retour aux administrateurs
  sendStatusChangeNotificationToAdmin: async (
    commande,
    client,
    admins,
    newStatus
    ...
    try {
      ...
      console.error(
        "Erreur lors de l'envoi de la notification de changement de statut:",
        error
      );
      return false;
    }
