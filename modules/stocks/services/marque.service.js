...
const getMarqueById=async(id)=>{
    const [result]=await db.select().from(marques).where(eq(marques.id_marque,id))
    return result
}

const updateMarque=async(id,data)=>{
    const [result]=await db.update(marques).set(data).where(eq(marques.id_marque,id)).returning()
    return result
}

const deleteMarque=async(id)=>{
    const [result]=await db.delete(marques).where(eq(marques.id_marque,id)).returning()
    return result
}
